...
struct completion mdss_dsi_panel_on_ok;
struct delayed_work panel_on_ok_work;
struct workqueue_struct *panel_on_ok_wq;
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if(panel_on_ok_wq)
	{
		queue_delayed_work(panel_on_ok_wq, &panel_on_ok_work,
							msecs_to_jiffies(1));
	}
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	printk("mdss_dsi_panel_off  \n");
	//cancel_delayed_work(&panel_on_ok_work);
	ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (data) {
		if (!strncmp(data, "bl_ctrl_wled", 12)) {
			...
			led_trigger_register_simple("bkl-trigger",
				&bl_led_trigger);
			pr_debug("#### %s: SUCCESS->BL_GPIO  TRIGGER register\n",
				__func__);
...
static void mdss_panel_panel_on_work(struct work_struct *work)
{
	msleep(220);
 	complete(&mdss_dsi_panel_on_ok);
}
	...
	bool cmd_cfg_cont_splash)
{
	...
	//baoqiang add end
	panel_on_ok_wq = create_singlethread_workqueue("mdss_panel_panel_on");
	if( panel_on_ok_wq )
	{
		INIT_DELAYED_WORK_DEFERRABLE(&panel_on_ok_work,mdss_panel_panel_on_work); 
