			...
			unsigned int cmd, unsigned long arg)
{
	...
	case ALPSIO_SS_SET_MAGACTIVATE:
		...
		if (tmpval){
			...
			//flgActivate |=  ACTIVE_SS_ORI;//mark
		}
		else{
			flgActivate &= ~ACTIVE_SS_MAG;
		//flgActivate &= ~ACTIVE_SS_ORI;//mark
	...
	case ALPSIO_SS_SET_DELAY:
		...
		delay = tmpval;
		if (flgActivate & (ACTIVE_SS_MAG | ACTIVE_SS_ORI))
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case ALPSIO_AD_GET_DATA:
		...
		if (ret < 0) {
		    //delete by junfeng.zhou.sz for delete the mislead error log with 578445 begin,20131224
			/*dev_err(&alps_idev->input->dev, ALPS_IDEV_LOG_TAG
				"error: "
				"wait_event_interruptible_timeout, %d\n", ret);*/
			//delete by junfeng.zhou.sz for delete the mislead error log with 578445 end
			return -EFAULT;
		}
		mutex_lock(&alps_lock);
		if (ret == 0)
...
static void alps_poll(struct input_polled_dev *dev)
{
	...
	if (!flgSuspend) {
		...
		if (flgActivate &
			...
			magsns_poll(idev);
		if (flgActivate & (ACTIVE_SS_ACC | ACTIVE_SS_ORI))
			accsns_poll(idev);
		if (flgActivate &
