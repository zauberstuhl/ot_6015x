...
struct tsl2772_chip {
	...
	bool als_enabled;
	bool prx_enabled;
    //bool als_pre_enabled;
	//bool prx_pre_enabled;
...
static void taos_calc_cpl(struct tsl2772_chip *chip)
{
	...
	cpl = (atime * chip->params.als_gain)/(GA * 24)/chip->params.als_gain;
	if (agl)
		cpl = cpl * 16 / 1000;
    ...
    /*printk(KERN_INFO "%s: cpl = %u [ gain %u, agl %u], "
			"saturation %u\n", __func__, cpl,
			chip->params.als_gain, agl, sat);*/
...
static int taos_get_lux(struct tsl2772_chip *chip)
{
    ...
    lux_0 = (c0 - 2 * c1) / cpl;
    lux_1 = (0.6 * c0 - c1) / cpl;
    //lux = max_t(u64,lux_0,lux_1);
    lux = ((lux_0 - lux_1)>0)?lux_0 : lux_1;
	...
	for (i = 0; i < chip->segment_num; i++, s++) {
		if (ratio <= s->ratio) {
            ...
            printk(KERN_INFO "%s: ratio %u segment %u " 
					"[r %u, k0 %u, k1 %u]\n", __func__,
					ratio, i, s->ratio, s->k0, s->k1); 
			k0 = s->k0;//95264
			k1 = s->k1;//195340
...
exit:
    ...
    /*printk(KERN_INFO "%s: lux %u ( c0:%u -  c1: %u) / cpl %u\n",
		__func__, lux,  c0, c1, cpl);*/
...
static int taos_read_all(struct tsl2772_chip *chip)
{
    ...
    /*for(;i<(TSL277X_PRX_HI - TSL277X_STATUS + 1);i++)
        {
            printk(KERN_INFO "%s :shadow[0x%x] 0x%x",__func__,(TSL277X_STATUS+i),chip->shadow[TSL277X_STATUS+i]);
        }*/
	return (ret < 0) ? ret : 0;
}
...
/*static int taos_check_and_report(struct tsl2772_chip *chip)
{
    ...
    wake_lock_timeout(&chip->taos_wakelock, 3*HZ);
...
{
    struct tsl2772_chip *chip = container_of(work, struct tsl2772_chip, taos_work);
...
{
	struct tsl2772_chip *chip = container_of(timer, struct tsl2772_chip, ps_timer);
    //flush_regs(chip);
	queue_work(chip->taos_ps_wq, &chip->taos_ps_work);
	hrtimer_forward_now(&chip->ps_timer, chip->ps_poll_delay);
	return HRTIMER_RESTART;
}

static void taos_ps_work_func(struct work_struct *work)
{
	struct tsl2772_chip *chip = container_of(work, struct tsl2772_chip, taos_ps_work);
    int ret;

    if(chip->prx_enabled != true)
    {
        msleep(500);
...
/*static irqreturn_t taos_irq(int irq, void *handle)
{
	...
	queue_work(chip->taos_wq,&chip->taos_work);
...
{
	struct taos_raw_settings const *s = chip->pdata->raw_settings; //s == NULL;
	u8 *sh = chip->shadow;
	struct device *dev = &chip->client->dev;
	...
	if (s) {
        ...
        sh[TSL277X_PRX_PULSE_COUNT] = 0x08;
		//sh[TSL277X_CONTROL] = AGAIN_8 | PGAIN_4 |
		//      PDIOD_CH0 | PDRIVE_120MA;
        sh[TSL277X_CONTROL] = AGAIN_1 | PGAIN_1 |
				...
				const char *buf, size_t size)
{
    ...
    /*if(value == chip->als_enabled)
        return 0;*/
    mutex_lock(&chip->lock);
    if (value){  
       /* if(chip->screen_status == 1)
    	    taos_als_enable(chip,1);
				...
				const char *buf, size_t size)
{
    ...
    /*if(value == chip->prx_enabled)
        return 0;*/
    mutex_lock(&chip->lock);
    if (value)
    {
    	//taos_prox_enable(chip,1);
        hrtimer_start(&chip->ps_timer, chip->ps_poll_delay, HRTIMER_MODE_REL);
...
{
    struct tsl2772_chip *chip = dev_get_drvdata(dev);
	return sprintf(buf, "%d %d\n", chip->pdata->parameters.prox_th_min, chip->pdata->parameters.prox_th_max);
}
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&chip->lock);
	chip->pdata->parameters.prox_th_min = det_val;
	chip->pdata->parameters.prox_th_max = hsyt_val;
	mutex_unlock(&chip->lock);
...
static struct device_attribute prox_attrs[] = {
	__ATTR(prx_raw, 0666, taos_device_prx_raw, NULL),
	__ATTR(prx_detect, 0666, taos_device_prx_detected, NULL),
	__ATTR(enable, 0666, tsl2772_prox_enable_show, tsl2772_prox_enable),
	__ATTR(ps_sensor_thld, 0666, taos_show_ps_sensor_thld, taos_store_ps_sensor_thld),
};
...
static struct device_attribute als_attrs[] = {
	...
	__ATTR(als_gain, 0666, taos_als_gain_show, taos_als_gain_store),
	__ATTR(als_gate, 0666, taos_als_gate_show, taos_als_gate_store),
	__ATTR(lux_table, 0666, taos_lux_table_show, taos_lux_table_store),
	__ATTR(enable, 0666, tsl2772_als_enable_show, tsl2772_als_enable),
	...
	const struct i2c_device_id *idp)
{
    ...
    pdata->als_can_wake = true;
    pdata->proximity_can_wake = true;
...
bypass_als_idev:
   ...
   chip->taos_als_wq = create_singlethread_workqueue("taos_als_wq");
	...
	chip->taos_ps_wq = create_singlethread_workqueue("taos_ps_wq");
	INIT_WORK(&chip->taos_ps_work, taos_ps_work_func);
	hrtimer_init(&chip->ps_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	chip->ps_poll_delay = ns_to_ktime(200 * NSEC_PER_MSEC);
	chip->ps_timer.function = taos_ps_timer_func;
...
static int __devexit taos_remove(struct i2c_client *client)
{
	...
	destroy_workqueue(chip->taos_als_wq);
    hrtimer_try_to_cancel(&chip->ps_timer);
    destroy_workqueue(chip->taos_ps_wq);
    wake_lock_destroy(&chip->taos_nosuspend_wl);
    wake_lock_destroy(&chip->taos_wakelock);
