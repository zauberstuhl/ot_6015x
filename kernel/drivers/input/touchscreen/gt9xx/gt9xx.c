...
/*******************************************************
Function:
	Send config data after rawdata test.
Input:
	client: i2c device.
Output:
...
*********************************************************/
s32 gtp_send_cfg_rawdata(struct i2c_client *client)
	{
		int ret;
		struct goodix_ts_data *ts = i2c_get_clientdata(client);
...
/*ADD -END--- optimize the EngineerMode  --- bug 554634 ---qifu.cheng added --- 14/3/26*/	  
	if (Rbuf[5] == 0x00) {
		...
		if (Rbuf[5] == 'S' || Rbuf[5] == 's')
			chip_gt9xxs = 1;
		num_read_chars =sprintf(buf,"reg=%d data=%d %c%c%c%c_%02x%02x\n", cfg_buf[0],grp_cfg_version, Rbuf[2],
