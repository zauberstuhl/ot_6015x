...
struct qpnp_chg_chip {
	...
	//add-bengin shiqian.zhou@t2mobile.com
	struct work_struct              temp_check_work;
	int				low_temp;
	int				high_temp;
	int				temp_monitor;
	//add-end
...
//add-bengin shiqian.zhou@t2mobile.com
static void
qpnp_bat_temp_check_work(struct work_struct *work);
//add-end
static inline int
						...
						enum power_supply_property psp)
{
	...
	/* add for customize enable/disabled charging
	 * author shiqian.zhou@t2mobile.com
	 */
	case POWER_SUPPLY_PROP_LOW_TEMP:
	case POWER_SUPPLY_PROP_HIGH_TEMP:
	case POWER_SUPPLY_PROP_TEMP_MONITOR:
		return 1;
	default:
		break;
	}
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	/* add for customize enable/disable charging
	 * author shiqian.zhou@t2mobile.com
	 */
	POWER_SUPPLY_PROP_LOW_TEMP,
	POWER_SUPPLY_PROP_HIGH_TEMP,
	POWER_SUPPLY_PROP_TEMP_MONITOR,
};
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
	/* add for customize enable/disable charing temp
	 * author shiqian.zhou@t2mobile.com
	 */
	case POWER_SUPPLY_PROP_LOW_TEMP:
		val->intval = chip->low_temp;
		break;
	case POWER_SUPPLY_PROP_HIGH_TEMP:
		val->intval = chip->high_temp;
		break;
	case POWER_SUPPLY_PROP_TEMP_MONITOR:
		val->intval = chip->temp_monitor;
		break;
	default:
		return -EINVAL;
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		...
		break;
	/* add for customize enable/disable charging 
	 * author shqian.zhou@t2mobile.com
	 */
	case POWER_SUPPLY_PROP_LOW_TEMP:
		chip->low_temp = val->intval;
		return rc;
	case POWER_SUPPLY_PROP_HIGH_TEMP:
		chip->high_temp = val->intval;
		return rc;
	case POWER_SUPPLY_PROP_TEMP_MONITOR:
		chip->temp_monitor = val->intval;
		if(chip->temp_monitor){
			schedule_work(&chip->temp_check_work);
			return rc;
		}
		else {
			cancel_work_sync(&chip->temp_check_work);
			power_supply_changed(&chip->batt_psy);
			return rc;
		}
	default:
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	 ...
	 * author shiqian.zhou@t2mobile.com
	 */
	chip->temp_monitor = 0; //default disable temp monitor;
	chip->low_temp = 2; //default disable temp monitor;
	chip->high_temp = 50; //default disable temp monitor;
	...
	if (chip->bat_if_base) {
		...
		//add-begin shiqian.zhou@t2mobile.com
		INIT_WORK(&chip->temp_check_work,qpnp_bat_temp_check_work);
		//add-end
	}
...
{
	struct qpnp_chg_chip *chip = container_of(work,
				struct qpnp_chg_chip,temp_check_work);
	int temp;
	int low_temp;
	int high_temp;
	temp = get_prop_batt_temp(chip);
	temp = temp/10;
	low_temp = chip->low_temp;
	high_temp = chip->high_temp;
	if(chip->charging_disabled == 1 && temp >= low_temp && temp <= high_temp){
		printk("%s:enable charging\n",__FUNCTION__);
		/* enable charging */  
	...
	else if(chip->charging_disabled == 0 &&(temp >= high_temp+5 || temp <= low_temp-2))
	{
		/* disable charging */  
		printk("%s:disable charging\n",__FUNCTION__);
		chip->charging_disabled = 1;	
		qpnp_chg_charge_en(chip,0);
		/* sleep for a second before disabling */
		msleep(1000);
		qpnp_chg_force_run_on_batt(chip,1);
		power_supply_changed(&chip->batt_psy);
	}
