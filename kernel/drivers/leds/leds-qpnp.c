...
extern struct completion mdss_dsi_panel_on_ok;
extern struct workqueue_struct *panel_on_ok_wq;

static int last_brightness = -1 ;
static int qpnp_gpio_set_one_wire(struct qpnp_led_data *led)
{
	static int initialized = 0; // 0:not initialize, 1:initialized
	unsigned char addr = 0x72; // 0111 0010
	int tl, th, bl_level, i;
	unsigned long flags;
	unsigned int  ret ;
	
	bl_level = (led->cdev.brightness) * BRIGHTNESS_MAX_SIZE / 256;
	if (bl_level >= BRIGHTNESS_MAX_SIZE) {
		bl_level = BRIGHTNESS_MAX_SIZE - 1;
	}

	if(panel_on_ok_wq && last_brightness == 0 && led->cdev.brightness != 0)
	{
		INIT_COMPLETION(mdss_dsi_panel_on_ok);
		 ret = wait_for_completion_timeout(&mdss_dsi_panel_on_ok,
			msecs_to_jiffies(1000));
	}
	
	last_brightness = led->cdev.brightness;
	
	// pr_warn("%s, brightness: %d, level:%d\n", __func__, led->cdev.brightness, bl_level);
	local_irq_save(flags);
	// Added by Xiaodong.Chen at 2014.1.23 for change initialize ic step
	if (bl_level > 0 && 0 == initialized) {
		...
		gpio_direction_output(led->gpio_cfg->gpio_num, 1);
		initialized = 1;
		pr_warn("%s, one_wire_initialized open\n", __func__);
	}
	else if (bl_level <= 0) {
		gpio_direction_output(led->gpio_cfg->gpio_num, 0);
		mdelay(3);
		initialized = 0;
		pr_warn("%s, one_wire_initialized close\n", __func__);
		goto EXIT;
	}
	// Added end
	// Address transfer
	...
	const char *buf, size_t count)
{
	...
	if (ret)
		return ret;
	led = container_of(led_cdev, struct qpnp_led_data, cdev);
	led->cdev.brightness = blinking ? led->cdev.max_brightness/2 : 0;
